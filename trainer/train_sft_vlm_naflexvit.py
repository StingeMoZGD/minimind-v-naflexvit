...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            Logger(f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}), loss: {current_loss:.4f}, logits_loss: {current_logits_loss:.4f}, aux_loss: {current_aux_loss:.4f}, lr: {current_lr:.8f}, epoch_time: {eta_min:.1f}min', log_file=log_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='sft_vlm_naflexvit1', type=str, help="保存权重的前缀名")
    ...
    parser.add_argument('--max_seq_len', default=832, type=int, help="训练的最大截断长度")
    parser.add_argument('--use_moe', default=1, type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument("--data_path", type=str, default="/data1/yym/MINIMIND/minimind-v-dataset/sft_i2t.parquet", help="训练数据路径")
    parser.add_argument('--from_weight', default='pretrain_vlm_naflexvit1', type=str, help="基于哪个权重训练，为none则不基于任何权重训练")
    ...
    Logger(f"{args.from_weight} start training...", log_file=log_file)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0: 
            Logger(f'Epoch [{epoch + 1}/{args.epochs}]: 跳过前{start_step}个step，从step {start_step + 1}开始', log_file=log_file)
            train_epoch(epoch, loader, len(loader) + skip, start_step, wandb, log_file=log_file)
        else:
