...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lora_save_path = f'{args.save_dir}/{args.lora_name}_{vlm_config.hidden_size}{moe_suffix}.pth'
            # LoRA只保存LoRA权重
            save_lora(model, lora_save_path)
            vlm_checkpoint(vlm_config, weight=args.lora_name, model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints')
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rank', default=32, type=int, help="LoRA秩值")
    parser.add_argument('--lora_target', default=1, type=int, choices=[0, 1, 2], help="lora目标层(0=所有qkv、mlp，1=仅llm层和投影层，2=仅视觉qkv和投影层)")
    parser.add_argument('--max_seq_len', default=340, type=int, help="训练的最大截断长度（中文1token≈1.5~1.7字符）")
    parser.add_argument('--use_moe', default=1, type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument("--data_path", type=str, default="/data1/yym/MINIMIND/minimind-v-dataset/pretrain_i2t.parquet", help="LoRA训练数据路径")
    parser.add_argument('--freeze_llm', default=1, type=int, choices=[0, 1, 2, 3], help="冻结策略（0=完全可训练，1=冻结+解冻首尾层，2=完全冻结仅训练proj，3=冻结所有层用于lora）")
    parser.add_argument('--from_weight', default='pretrain_vlm_naflexvit1', type=str, help="基于哪个权重训练，默认full_sft")
    ...
    ckp_data = vlm_checkpoint(vlm_config, weight=args.lora_name, save_dir='../checkpoints') if args.from_resume==1 else None
    ...
    apply_lora(model, rank=args.rank, lora_target=args.lora_target)
