...
def load_vlm_model(model_path):
    ...
    with model_lock:
        ...
        model.vision_encoder, model.processor = MiniMindVLM_naflexvit.get_vision_model(vision_model_path)
...
def pad_to_divisible(image, divisor=16, pad_value=0):
    """
    将 PIL 图片填充到宽高都能被 divisor 整除。
    参数:
        image: PIL.Image 对象
        divisor: 除数（默认16）
        pad_value: 填充值，0表示黑色 (0,0,0)，可以改为 (255,255,255) 白色或边缘扩展
    返回:
        填充后的 PIL.Image
    """
    width, height = image.size
    new_width = math.ceil(width / divisor) * divisor
    new_height = math.ceil(height / divisor) * divisor
    
    if width == new_width and height == new_height:
        return image  # 无需填充
    
    # 创建新画布，背景为 pad_value（RGB 模式）
    if isinstance(pad_value, int):
        pad_color = (pad_value, pad_value, pad_value)
    else:
        pad_color = pad_value
    
    new_image = Image.new("RGB", (new_width, new_height), pad_color)
    # 将原图粘贴到左上角
...
def chat(prompt, current_image_path=None):
    ...
    if current_image_path:
        ...
        pixel_values = {k: v.to(model.device) for k, v in MiniMindVLM_naflexvit.image2tensor_web(image, preprocess).items()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_from', default='/data1/yym/MINIMIND/minimind-v-master', type=str, help="transformers模型扫描目录")
