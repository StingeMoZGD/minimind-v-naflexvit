...
def parse_special_tokens(text):
    ...
    if '<|box_start|>' in text and '<|box_end|>' in text:
        start = text.find('<|box_start|>') + len('<|box_start|>')
        end = text.find('<|box_end|>')
        box_str = text[start:end]
        # 可以返回坐标，也可以替换成更友好的显示
        return f"检测到目标框: {box_str}"
    else:
        return text
...
def launch_gradio_server(server_name="0.0.0.0", server_port=7788):
    ...
    def respond(message, history):
        ...
        for chunk in chat(text, img_path):
            response += chunk
            parsed = parse_special_tokens(response)
            yield history + [{"role": "assistant", "content": parsed}]
